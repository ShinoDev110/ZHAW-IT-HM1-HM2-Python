# ============================================================
# TOPIC: ODE — Rocket ascent via cumulative trapezoidal rule
# DESCRIPTION:
# From a given acceleration function a(t), v(t) = int_0^t a(s) ds
# and h(t) = int_0^t v(s) ds are computed via the cumulative trapezoidal
# rule, optionally compared against the analytical solution.
# USE WHEN:
# When velocity and altitude are to be obtained from a(t) successively
# by quadrature instead of an ODE solver (typical for rocket/braking problems).
# EXAMPLE:
...
t_E   = 190.0
g     = 9.81
mu    = (m_A - m_E) / t_E    # constant mass flow rate

def a(t):
    return v_rel * mu / (m_A - mu * t) - g

# Analytical reference solutions (set to None if not available)
def v_exact(t):
